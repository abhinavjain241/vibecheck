...
async def callback(code: str):
    ...
    events = get_events("2024-10-12", "2024-10-13")
    ...
    for event in events:
        ...
        for artist_id, artist_name in event["artists"]:
            spotify_artist_id = get_or_create_spotify_artist(spotify, artist_id, artist_name)
        ...
        event_image_url = event.get("image_url")  # Assuming the event dictionary has an image_url key
        event_genre_histogram = {genre: count for genre, count in event_genres[event_title].items()}
    ...
    user_genre_histogram = {genre: user_genres[genre] for genre in list(user_genres.keys())[:10]}  # Clip to max 10 genres
